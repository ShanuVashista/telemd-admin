...
export function AuditLogs() {
  ...
  const handleClickOpen = (selectedPatientId) => {
    let selectedPatient = auditLog.find(
      (singleDocotor) => singleDocotor.id === selectedPatientId
    );
  ...
  const getAuditLogs = async () => {
    try {
      ...
      //   page,
      //   limit: 10,
      //   cond: { role_id: "patient", search: searchValue },
      // });
    ...
    <Grid className="p-3" container direction="column" wrap="nowrap">
      <Grid container wrap="nowrap" direction="column" component={Paper}>
              ...
              {/* {patients.map((data, i) => (
                <tr key={i} style={{ verticalAlign: "middle" }}>
                  ...
                  <td className="text-right">
                    <IconButton
                      size="small"
                      onClick={(event) => openMenu(event, data.id)}
                    >
        ...
        <Menu anchorEl={anchorEl} open={open} onClose={close}>
          <MenuItem onClick={(e) => handleClickOpen(selectedPatientId)}>
            View
          </MenuItem>
