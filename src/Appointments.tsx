...
export function Appointments() {
  ...
  const handleClickOpen = (selectedAppointmentId) => {
    let selectedAppointmentDetails = appointments.find(
      (singleAppointment) => singleAppointment._id === selectedAppointmentId
    );
    ...
    <Grid className="p-3" container direction="column" wrap="nowrap">
      <Grid container wrap="nowrap" direction="column" component={Paper}>
            ...
            <tbody>
              {appointments.map((data, i) => (
                <tr key={i} style={{ verticalAlign: "middle" }}>
                  ...
                  <td>
                    {moment(data.dateOfAppointment).format("DD-mm-YYYY HH:mm")}
                  </td>
                  <td>{data.status}</td>
                  <td className="text-center">
                    {data.isEmergency ? (
                      <Chip
                        label="Yes"
                        style={{ backgroundColor: "#ee57579e" }}
                      />
                    ) : (
                      <Chip
                        label="No"
                        style={{ backgroundColor: "#e8e82e78" }}
                      />
                    )}
                  </td>
        ...
        <Menu anchorEl={anchorEl} open={open} onClose={close}>
          <MenuItem onClick={(e) => handleClickOpen(selectedAppointmentId)}>
            View
          </MenuItem>
